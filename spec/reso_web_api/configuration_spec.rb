RSpec.describe ResoWebApi::Client do
  describe 'default settings' do
    it 'returns the default configuration' do
      ...
      expect(ResoWebApi.auth_endpoint).to be_nil
      expect(ResoWebApi.endpoint).to be_nil
  ...
  describe 'instance configuration' do
    let(:config) { {
      ...
      :auth_endpoint => 'https://auth.reso.org',
      :endpoint      => 'https://odata.reso.org',
      :user_agent    => 'RSpec'
    } }
  ...
  describe 'block configuration' do
    it 'should correctly set up the client' do
      ResoWebApi.configure do |config|
        ...
        config.auth_endpoint = 'https://auth.reso.org'
        config.endpoint      = 'https://odata.reso.org'
        config.user_agent    = 'RSpec'
      end

      expect(ResoWebApi.api_key).to eq('test key')
      expect(ResoWebApi.api_secret).to eq('T0pS3cr3t')
      expect(ResoWebApi.auth_endpoint).to eq('https://auth.reso.org')
      expect(ResoWebApi.endpoint).to eq('https://odata.reso.org')
